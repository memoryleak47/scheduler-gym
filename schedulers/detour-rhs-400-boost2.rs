    ...
    for (rw_i, rw) in ctxt.rws.iter().enumerate() {
        ...
        for m in sched.search_rewrite(ctxt.runner.iterations.len(), &ctxt.runner.egraph, rw) {
            ...
            for subst in m.substs {
                ...
                let mut detour_cost = cx_cost + pat_cost;

                // boost: rewrites that improve the detour cost of a class are prefered.
                let cc = ex.find_best_cost(lhs);
                if pat_cost < cc {
                    detour_cost = detour_cost.saturating_sub(3 * (cc - pat_cost));
                }
