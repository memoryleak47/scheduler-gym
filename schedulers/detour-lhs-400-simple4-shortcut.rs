...
fn sorted_push<T: Ord>(vec: &mut Vec<T>, item: T) {
    let index = match vec.binary_search(&item) {
        Ok(i) | Err(i) => i,
    };
    vec.insert(index, item);

    // poor mans implementation to test against:
    //   vec.push(item);
    //   vec.sort();
}
