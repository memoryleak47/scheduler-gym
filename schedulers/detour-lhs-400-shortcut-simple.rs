    ...
    for (id, detour) in classes {
        ...
        for (rw_i, rw) in rws.iter().copied() {
            ...
            for subst in submatches {
                ...
                let cx_cost = *ctxt_cost.get(&id).unwrap_or(&ctxt.cfg.unreachable_cost); // this is the cost you get from not being able to reach any root.
                let detour_cost = cx_cost + pat_cost;

                let rmatches = if is_special { &mut special_matches } else { &mut matches };
                sorted_push_by_key(rmatches, (detour_cost, rw_i, id, subst), |x| x.0);
